...
<html>
	...
	<body>
		...
		<div>
			<form method="post">
				...
				<select name = 'country'>
					...
					?>
					<input type="submit" name="submit-country" value="This One!">
				</select>
			</form>
			<form method = "post">
				...
				<select name = 'university'>
					<?php 
						...
						$sql = "SELECT Universities.Name FROM Universities LEFT JOIN UniversityCountry ON Universities.ID = UniversityCountry.UniversityID LEFT JOIN COUNTRIES ON UniversityCountry.CountryID = Countries.ID WHERE Countries.ID = '$country'";
						$result = $conn -> query($sql);
		...
		<div>
			<?php 
				echo "<h3>About " . $_SESSION["username"] . "</h3>";

				$id = $_SESSION["id"];
				$sql = "CALL GetUserTextbooks('$id')";
				$result = $conn->query($sql);
